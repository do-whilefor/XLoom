...
语义入口先扩展每个原始查询组，再为当前 Wiki 判断生成中英文检索问题，并对候选
...
仅处理新增／变更项并清理已移除项。查询改写和候选评分存入 `semantic`，最多保留
...
当前研究轮次；已有显式运行上限仍生效，并保留最后一次提交请求。完整判断不截断，
每批最多 8 项，独立批次最多 2 个并行。超模型容量、无模型、非法响应或调用失败时
