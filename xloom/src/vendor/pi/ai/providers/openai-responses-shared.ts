...
import { parseToolArguments } from "../utils/stream-integrity.js";
...
export function convertResponsesMessages<TApi extends Api>(
	...
	for (const msg of transformedMessages) {
		if (msg.role === "user") {
            ...
            if (isSameModel && assistantMsg.providerData?.type === "openai-responses") {
                messages.push(...assistantMsg.providerData.output as unknown as ResponseInput);
                continue;
            }
...
export async function processResponsesStream<TApi extends Api>(
    ...
    let refused = false;
    const completedItems = new Map<number, string>();
    const nativeItems = new Map<number, ResponseOutputItem>();
    const startedItems = new Map<number, ResponseOutputItem>();
	...
	const createSlot = (outputIndex: number, item: ResponseOutputItem): ResponsesOutputSlot | undefined => {
        if (completedItems.has(outputIndex) || outputSlots.has(outputIndex)) throw new Error("Duplicate Responses output item index");
        startedItems.set(outputIndex, structuredClone(item));
	...
	const finalizeResponse = (
        ...
        const finalItems = response.output?.length ? response.output : [...nativeItems.entries()].sort((a, b) => a[0] - b[0]).map(([, item]) => item);
        output.providerData = { type: "openai-responses", output: structuredClone(finalItems) as unknown as import("../types.js").JsonValue[] };
		if (response?.id) {
		...
		if (response?.usage) {
            ...
            if (response.usage.input_tokens !== undefined) available.push("input");
            if (response.usage.output_tokens !== undefined) available.push("output");
            if (inputDetails?.cached_tokens !== undefined) available.push("cacheRead");
            if (inputDetails?.cache_write_tokens !== undefined) available.push("cacheWrite");
            if (response.usage.output_tokens_details?.reasoning_tokens !== undefined) available.push("reasoning");
            if (response.usage.total_tokens !== undefined || (response.usage.input_tokens !== undefined && response.usage.output_tokens !== undefined)) available.push("totalTokens");
            output.usage = { available,
				// OpenAI includes cached and cache-write tokens in input_tokens, so subtract both.
				input: Math.max(0, (response.usage.input_tokens || 0) - cachedTokens - cacheWriteTokens),
				output: response.usage.output_tokens || 0,
				cacheRead: cachedTokens,
				cacheWrite: cacheWriteTokens,
				...(response.usage.output_tokens_details?.reasoning_tokens === undefined ? {} : { reasoning: response.usage.output_tokens_details.reasoning_tokens }),
				totalTokens: response.usage.total_tokens ?? (response.usage.input_tokens + response.usage.output_tokens),
				cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0, total: 0 },
			};
		}
        // Preserve billed usage before reporting a protocol integrity failure.
        if (response.model && response.model !== model.id) throw new Error("Provider returned an unexpected model");
        if (response.status === "completed") {
            if (outputSlots.size) throw new Error("Response completed with unfinished output items");
            const doneItems = [...nativeItems.entries()].sort((a, b) => a[0] - b[0]).map(([, item]) => item);
            if (doneItems.length !== finalItems.length) throw new Error("Response terminal output differs from completed items");
            for (let index = 0; index < finalItems.length; index++) {
                const item = finalItems[index], done = doneItems[index];
                if (!done || done.type !== item.type || done.id !== item.id) throw new Error("Response terminal item identity differs from completed items");
                if ((item.type === "function_call" || item.type === "message") && item.status !== "completed") throw new Error("Response contains an incomplete output item");
                if (item.type === "function_call" && done.type === "function_call" &&
                    (done.call_id !== item.call_id || done.name !== item.name || done.arguments !== item.arguments)) {
                    throw new Error("Response terminal function call differs from completed items");
                }
                if (item.type === "message" && done.type === "message" &&
                    (done.phase !== item.phase || JSON.stringify(done.content) !== JSON.stringify(item.content))) {
                    throw new Error("Response terminal message differs from completed items");
                }
            }
        }
        backfillReasoningSignatures(finalItems);
		...
		const mappedStop = refused ? { stopReason: "error" as const, errorMessage: "Provider refused to complete the request" } : mapStopReason(status, incompleteReason);
	...
	for await (const event of openaiStream) {
        // Retransmitted events have the same protocol sequence number. Never replay their deltas.
        const sequence = (event as { sequence_number?: number }).sequence_number;
        if (sequence !== undefined) {
            const serialized = JSON.stringify(event);
            const seen = seenSequence.get(sequence);
            if (seen !== undefined) {
                if (seen !== serialized) throw new Error("Conflicting Responses event sequence number");
                continue;
            }
            seenSequence.set(sequence, serialized);
        }
        if (sawTerminalResponseEvent) throw new Error("Unexpected event after terminal Responses event");
		if (event.type === "response.created") {
			output.responseId = event.response.id;
            if (event.response.model && event.response.model !== model.id) throw new Error("Provider returned an unexpected model");
            ...
            const completed = completedItems.get(event.output_index);
            if (completed !== undefined) {
                if (completed !== serializedItem) throw new Error("Responses item changed after completion");
                continue;
            }
            const started = startedItems.get(event.output_index);
            if (started && (started.type !== item.type || started.id !== item.id ||
                (started.type === "function_call" && item.type === "function_call" &&
                    (started.call_id !== item.call_id || started.name !== item.name)))) {
                throw new Error("Responses output item identity changed during stream");
            }
            nativeItems.set(event.output_index, structuredClone(item));
            if (item.type === "message" && item.content.some(c => c.type === "refusal")) refused = true;
            applyMessagePhaseStopReason(item);
			...
			if (item.type === "reasoning" && slot?.type === "thinking") {
				...
				slot.block.arguments = item.status === "completed"
                ? parseToolArguments(item.arguments || slot.block.partialJson)
		    ...
		    completedItems.set(event.output_index, serializedItem);
...
function mapStopReason(
	...
	if (!status) return { stopReason: "error", errorMessage: "Response terminal event has no status" };
	switch (status) {
		case "completed":
		...
		case "in_progress":
		case "queued":
			return { stopReason: "error", errorMessage: `Response terminal event has nonterminal status: ${status}` };
		default: {
