...
import type { AgentMessage, StreamFn, ThinkingLevel } from "../../../agent/types.js";
import { uuidv7 } from "../../../ai/index.js";
import type { AssistantMessage, Context, Model, SimpleStreamOptions, Usage } from "../../../ai/types.js";
import { contentText } from "./utils.js";
...
export function findCutPoint(
	...
	for (let i = endIndex - 1; i >= startIndex; i--) {
		...
		if (accumulatedTokens >= keepRecentTokens) {
			// A large final tool result has no later valid cut point. Keep its
			// preceding assistant call instead of falling back to the first turn.
			cutIndex = cutPoints.findLast((point) => point <= i) ?? cutPoints[0];
...
/**
 * Shared summarization call through XLoom's configured provider stream.
 * Upstream provider discovery and retry policy are omitted; errors pause the caller.
 */
export async function completeSummarization(
	...
	if (!streamFn) throw new Error("XLoom summary requires the configured provider stream");
	requestOptions.signal?.throwIfAborted();
	return (await streamFn(model, context, requestOptions)).result();
}
...
export async function generateSummaryWithUsage(
	...
	if (!textContent.trim()) throw new Error("Summarization returned an empty summary");
