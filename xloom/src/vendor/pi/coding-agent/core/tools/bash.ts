...
export function createShellTool(
	...
	return {
		...
		async execute(
			...
			const spawnContext = { command: resolvedCommand, cwd, env: getShellEnv() };
			...
			try {
				...
				try {
					...
					if (err instanceof Error && err.message === "aborted") {
						throw Object.assign(new Error(appendStatus(text, "Command aborted")), { details: { ...details, exitCode: null, status: "interrupted", outcome: "unknown" } });
					}
					if (err instanceof Error && err.message.startsWith("timeout:")) {
						const timeoutSecs = err.message.split(":")[1];
						throw Object.assign(new Error(appendStatus(text, `Command timed out after ${timeoutSecs} seconds`)), { details: { ...details, exitCode: null, status: "interrupted", outcome: "unknown" } });
					}
					throw err;
				}
				...
				if (exitCode === null) throw Object.assign(new Error(appendStatus(outputText, "Command terminated by signal; outcome unknown")), { details: { ...details, exitCode, status: "interrupted", outcome: "unknown" } });
				if (exitCode !== 0) {
					throw Object.assign(new Error(appendStatus(outputText, `Command exited with code ${exitCode}`)), { details: { ...details, exitCode } });
				}
...
export function createBashTool(
	...
	return createShellTool(cwd, bashToolConfig, options);
}
