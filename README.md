...
- Claude 风格的暖珊瑚色极简会话：用户 `❯`、回复 `●`，正文支持 Markdown；Decide / Execute / Decide · Meta 交接只占一行，工具默认显示名称、目标与状态，不刷文件正文或原始 JSON。
...
| `/details`、Ctrl+O | 展开 / 收起工具输入输出、交接原因及原始协议；详情有长度限制，不影响原始运行日志 |
| `/help` | 查看命令与快捷键帮助 |
...
普通聊天继续流式显示自然语言；双 Agent 的原始结果协议默认收起，只有控制器成功提交黑板后的摘要才显示为结果，工具成功不等于目标完成。工具错误仍直接可见；端点未知定价警告每个 TUI 会话只提示一次。`/` 列表只提供应用已有命令，不新增工具、MCP 或 Skills。候选中的 `/model` 也支持补全角色。
