...
export interface MaterialCard {
  key: string; signature: string; change: "new" | "changed"; kind: string; id: string; title: string; readPath: string;
...
export interface MaterialDelivery {
  ...
  baseline: "initial" | "previous_planning"; items: MaterialCard[]; added: number; changed: number; unchanged: number;
...
export function materialDelivery(board: BoardSnapshot, baseline: Record<string, string>, budgetChars = 6000,
  originalMatches: Map<string, Set<string>> = new Map(), originalLinking?: MaterialDelivery["originalLinking"]): MaterialDelivery {
  ...
  let unchanged = 0;
  for (const doc of retrievalDocuments(board).documents) {
  ...
  candidates.sort((a, b) => Number(b.kind === "gap") - Number(a.kind === "gap") || Number(!!b.relatedGaps.length) - Number(!!a.relatedGaps.length)
  ...
  const result: MaterialDelivery = { generator: wikiGenerator, type: "planning_materials", evidence: false, boardRevision: board.revision,
    baseline: Object.keys(baseline).length ? "previous_planning" : "initial", items: [], added: 0, changed: 0, unchanged, deferredCount: 0, deferred: [],
  ...
  for (const card of candidates) {
    ...
    if (card.change === "new") result.added++; else result.changed++;
...
export function planningMaterials(board: BoardSnapshot, baseline: Record<string, string>, dataDir: string, workspace: string, budgetChars = 6000) {
  const changedEvidence = retrievalDocuments(board).documents.some(doc => doc.ref.kind === "evidence" && baseline[refKey(doc.ref)] !== wikiDigest(doc));
