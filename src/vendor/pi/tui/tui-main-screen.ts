...
export class TuiMainScreen extends TuiBase implements TUI {
	...
	protected doRender(): void {
		...
		const logRedraw = (_reason: string): void => {};
		...
		for (let i = firstChanged; i <= renderEnd; i++) {
			...
			if (!isImage && visibleWidth(line) > width) {
                // XLoom diagnostics never contain rendered conversation text.
                const crashLogPath = path.join(os.homedir(), ".xloom", "logs", "tui-crash.log");
                const crashData = JSON.stringify({ timestamp: new Date().toISOString(), width, line: i, visibleWidth: visibleWidth(line) }) + "\n";
                fs.mkdirSync(path.dirname(crashLogPath), { recursive: true, mode: 0o700 });
