...
/** Reserve the last allowed model turn for reporting, without extending limits
 * or replaying any tool. Resource/time cancellation always takes precedence. */
export function createRunBudget(limits: ProjectConfig["limits"], usage: Usage, spent: Usage,
  ...
  const instruction = `Invocation budget: maxTurnsPerRun=${limits.maxTurnsPerRun}, including a reserved final reporting turn. At most ${limits.maxTurnsPerRun - 1} model turns may use tools. Save necessary artifacts before the final turn. This invocation's end does not mean the user's Goal is complete.`;
  const shouldStopAfterTurn: NonNullable<AgentOptions["shouldStopAfterTurn"]> = ({ message }) => {
    ...
    const reason = limits.maxTokens !== null && tokens >= limits.maxTokens ? `maxTokens=${limits.maxTokens}, tokens=${tokens}`
      ...
      : turns >= limits.maxTurnsPerRun ? `maxTurnsPerRun=${limits.maxTurnsPerRun}, turns=${turns}` : undefined;
  ...
  const prepareNextTurnWithContext: NonNullable<AgentOptions["prepareNextTurnWithContext"]> = ({ context }) => {
    ...
    if (turns !== limits.maxTurnsPerRun - 1) return;
  ...
  return {
    instruction: `${instruction}${limits.maxTurnsPerRun === 1 ? `\n${finalInstruction}` : ""}`,
    toolsAllowed: limits.maxTurnsPerRun > 1,
    shouldStopAfterTurn,
