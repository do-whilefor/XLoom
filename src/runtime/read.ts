...
export function createWorkspaceReadTool(workspace: string, artifactsDirectory?: string, task?: TaskReadContext) {
  ...
  if (readTask) tool.description += " Wiki files are derived, not evidence originals. Follow evidence.originalReadPath for verified archive bytes, nextReadPath for continuation. reading reports same-role overlap and remaining original bytes; do not reopen already delivered Wiki sources merely in another view. Delivery is not review.";
