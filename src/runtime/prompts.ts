...
const checkpointProtocol = `Checkpoints: write complete JSON to checkpointFile: {id:"unique-batch-id",execution:{same contract},yieldToDecide:false}. Evidence: only ref/path/description. Rejected writes create no file; rewrite, never edit checkpointFile. Controller acceptance commits; reuse returned IDs/keys. Submit uncommitted records only. Last call yieldToDecide:true requests planning, not Goal completion.`;
