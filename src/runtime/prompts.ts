...
const decisionProtocol = `Output (omit unused; omit conclusion while work remains; goals: new IDs only):
...
Choose one value per | enum. Priority is an integer 0–1000, higher first. Copy Fact IDs from facts/factIndex; never change ID prefixes. updateSteps changes ready Steps only; done/no_progress/blocked/failed/abandoned are history. For more work, inspect results and propose a new Step. Resolve pending Steps and active child Goals before satisfying a Goal. Only fresh metacog may conclude or satisfy the root: pair non-NEED_INPUT conclusion with root satisfied and supporting factIds. Never abandon the root Goal. One finding does not complete a multi-part Goal. conclusion.reason: concise Markdown in the user's language with results, evidence and remaining work; explain how the whole Goal is met.
...
Choose one value per | enum; omit unknown impact. New finding keys require target; for an existing key, omit target to retain it or copy it exactly. Put new observations in facts/next. Findings inherit their Facts' evidence; Execute cannot rate, verify or close them. Evidence: regular files in this run's artifacts with actual requests/responses, identity/object comparisons, state and reproduction details. Synthetic narratives are not evidence; keep large bodies in files.
...
If checkpointFile exists, write {id:"unique batch ID",execution:{same contract; new records only},yieldToDecide:false} there after useful work. Only controller acceptance commits evidence; reuse returned IDs/keys. Continue, or yieldToDecide:true on the last tool call requests fresh planning, not Goal completion. Final output: uncommitted records only.`;
