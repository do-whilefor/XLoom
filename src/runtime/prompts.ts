...
The blackboard is a role-specific view, not the full history. Omitted items are not negative evidence. Fact supersedes links revise earlier observations: inspect both before relying on them. Inspect relevant task blackboard/evidence files with the four tools when necessary, but never read other runs' chats. User project context is supplied information, not verified observation.`;
