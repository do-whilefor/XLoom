...
Choose one value per | enum. Omit unknown impact as a whole. Evidence must be regular files inside this run's artifacts, retaining actual requests/responses, identity/object comparisons, state results and reproduction details. A synthetic narrative is not original evidence. Keep large bodies in files. Findings inherit their Facts' supporting evidence. Execute cannot rate, verify or close findings.
