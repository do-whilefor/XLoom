...
Blackboard omissions are not negative evidence; user context is unverified. Check factIndex evidence and supersedes for older capabilities. Nest Step conditions in combination:{requires:[Fact IDs, merged into from],missing:[unverified conditions],scope:"identity/object boundary",stateVersion:"environment/session",expectedCapability:"joint result",counterEvidence:[contradictory Fact IDs]}; never put them at Step top level. Check identity/state compatibility; preserve partial capabilities; failed conditions do not disprove other combinations.`;
...
const checkpointProtocol = `Checkpoints: write complete JSON to checkpointFile: {id:"unique-batch-id",execution:{same contract},yieldToDecide:false}. Rejected writes create no file; rewrite, never edit checkpointFile. Only controller acceptance commits evidence; reuse returned IDs/keys. Submit new, uncommitted records only. Set yieldToDecide:true on the last tool call for fresh planning, not Goal completion.`;
